...
def sparsify_matrix(mat: Tensor, p: float = 1.0) -> Tensor:
    ...
    new_mat = mat.clone()
    new_mat[torch.rand_like(mat) > p] = 0.0
...
class FLIF_B(Module):
    def __init__(
        ...
        dtype=None,
        dt=1e-3,
        spike_func: Callable = SurrGradSpike.apply,
        recurrent: bool = True,
        train_v0: bool = False,
        return_V: bool = False,
        train_threshold: bool = False,
        threshold_std: float = 0.0,
        threshold_delta_max: float = 1.0,
        train_threshold_tau: bool = False,
        threshold_tau_mean: float = 0.99,
        threshold_tau_std: float = 0.0,  # look up plausible values
        train_V_tau: bool = False,
        V_tau_mean: float = 10e-3,
        V_tau_std: float = 0.0,
        train_I_tau: bool = False,
        I_tau_mean: float = 5e-3,
        I_tau_std: float = 0.0,
        ...
        self.input_con = nn.Linear(
            in_features, out_features, False, device, dtype
        )  # make sparse?
        self.input_dropout = input_dropout
        self.input_density = input_density
        if self.recurrent:
            self.recurrent_con = nn.Linear(
                out_features, out_features, False, device, dtype
            )  # make sparse or factorize?
        ...
        self.V_0 = Parameter(
            torch.empty(out_features, **factory_kwargs), requires_grad=self.train_V0
        )
        self.threshold = Parameter(
            torch.empty(out_features, **factory_kwargs),
            requires_grad=self.train_threshold,
        )
        self.register_buffer(
            "threshold_delta", torch.empty(out_features, **factory_kwargs)
        )
        self.threshold_tau = Parameter(
            torch.empty(out_features, **factory_kwargs),
            requires_grad=self.train_threshold_tau,
        )  # threshold decay time constant
        self.V_tau = Parameter(
            torch.empty(out_features, **factory_kwargs), requires_grad=self.train_V_tau
        )
        self.I_tau = Parameter(
            torch.empty(out_features, **factory_kwargs), requires_grad=self.train_I_tau
        )
    ...
    def reset_parameters(self) -> None:
        ...
        if self.input_density < 1.0:
            self.input_con.weight.data = sparsify_matrix(
                self.input_con.weight.data, self.input_density
        ...
        if self.recurrent:
            ...
            if self.recurrent_density < 1.0:
                self.recurrent_con.weight.data = sparsify_matrix(
                    self.recurrent_con.weight.data, self.recurrent_density
                )  # TODO: make this a hyperparamter
        init.zeros_(self.V_0)
        if self.threshold_std > 0.0:
            init.trunc_normal_(
                self.threshold, mean=1.0, std=self.threshold_std, a=0.5, b=1.5
            )
        else:
            init.ones_(self.threshold)
        init.uniform_(
            self.threshold_delta, 0, self.threshold_delta_max
        )  # replace 0 by - self.threshold_delta_max?
        if self.threshold_tau_std > 0.0:
            init.trunc_normal_(
                self.threshold_tau,
                mean=self.threshold_tau_mean,
                std=self.threshold_tau_std,
                a=0.0,
                b=1.0,
            )
        else:
            init.constant_(self.threshold_tau, self.threshold_tau_mean)
        if self.V_tau_std > 0.0:
            init.trunc_normal_(
                self.V_tau, mean=self.V_tau_mean, std=self.V_tau_std, a=0.0, b=1.0
            )
        else:
            init.constant_(self.V_tau, self.V_tau_mean)
        if self.I_tau_std > 0.0:
            init.trunc_normal_(
                self.I_tau, mean=self.I_tau_mean, std=self.I_tau_std, a=0.0, b=1.0
            )
        else:
    ...
    def init_state(self, batch_size: int) -> None:
        ...
        self.spk = torch.zeros(
            (batch_size, self.out_features), device=self.V_0.device
        )  # change dtype ?
    ...
    def forward(self, inpt: Tensor) -> Tensor:
        ...
        if self.recurrent:
            I_rec = F.dropout(
                self.recurrent_con(self.spk), self.recurrent_dropout, True, False
            )
        ...
        self.V_t = (
            self.V_t - self.spk.detach() * self.threshold_eff
        )  # subtractive method

        # adjust threshold if spiked
        self.threshold_eff = (
            self.threshold_tau * (self.threshold_eff - self.threshold)
            + self.threshold
            + self.threshold_delta * self.spk
        )
...
class TransitionNetRSNNPB(Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(num_rec_layers):
            ...
            layers[f"lif_rec{i+1}"] = FLIF_B(
                dim, hidden_dim, bias=bias, dt=dt, **factory_kwargs, **rec_flif_kwargs
            )
        for i in range(num_ff_layers):
            dim = in_dim if (i == 0 and num_rec_layers == 0) else hidden_dim
            layers[f"lif_ff{i+1}"] = FLIF_B(
                dim, hidden_dim, bias=bias, dt=dt, **factory_kwargs, **ff_flif_kwargs
            )
        layers["mu"] = Readout(
            hidden_dim, state_dim, dt=dt, **factory_kwargs, **readout_kwargs
        )
        layers["logvar"] = Readout(
            hidden_dim, state_dim, dt=dt, **factory_kwargs, **readout_kwargs
        )
    ...
    def forward(self, state: Tensor, action: Tensor) -> Union[Tensor, Tensor]:
        ...
        logvar_outs = torch.empty(
            (T, N, D), device=next(self.basis.logvar.parameters()).device
        )
        for t in range(T):
            mus, logvars = [], []
...
class PolicyNetRSNNPB(Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(num_rec_layers):
            dim = in_dim if i == 0 else hidden_dim
            layers[f"lif_rec{i + 1}"] = FLIF_B(
                dim, hidden_dim, dt=dt, **factory_kwargs, **rec_flif_kwargs
            )
        for i in range(num_ff_layers):
            dim = in_dim if (i == 0 and num_rec_layers == 0) else hidden_dim
            layers[f"lif_ff{i + 1}"] = FLIF_B(
                dim, hidden_dim, dt=dt, **factory_kwargs, **ff_flif_kwargs
            )
        layers["mu"] = Readout(
            hidden_dim, action_dim, dt=dt, **factory_kwargs, **readout_kwargs
        )
        layers["logvar"] = Readout(
            hidden_dim, action_dim, dt=dt, **factory_kwargs, **readout_kwargs
        )
    ...
    def step(self, state: Tensor, target: Tensor) -> Union[Tensor, Tensor]:
    ...
    def forward(self, state: Tensor, target: Tensor) -> Union[Tensor, Tensor]:
        ...
        mu_outs = torch.empty(
            (T, N, self.action_dim), device=next(self.basis.mu.parameters()).device
        )
        logvar_outs = torch.empty(
            (T, N, self.action_dim), device=next(self.basis.logvar.parameters()).device
        )
        for t in range(T):
            mus, logvars = [], []
            for i in range(self.repeat_input):
...
class PolicyNetRSNNPB_snntorch(Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(num_rec_layers):
            dim = in_dim if i == 0 else hidden_dim
            layers[f"lif_rec_in{i + 1}"] = nn.Linear(
                dim, hidden_dim, flif_kwargs.get("bias", True), **factory_kwargs
            )
        ...
        for i in range(num_ff_layers):
            dim = in_dim if (i == 0 and num_rec_layers == 0) else hidden_dim
            layers[f"lif_ff_in{i + 1}"] = nn.Linear(
                dim, hidden_dim, flif_kwargs.get("bias", True), **factory_kwargs
            )
        ...
        layers["mu_in"] = nn.Linear(
            hidden_dim,
            action_dim * n_pop,
            readout_kwargs.get("bias", True),
            **factory_kwargs,
        )
        ...
        nn.init.constant_(layers["mu_out"].weight, 1.0 / n_pop)
        layers["logvar_in"] = nn.Linear(
            hidden_dim,
            action_dim * n_pop,
            readout_kwargs.get("bias", True),
            **factory_kwargs,
        ...
        nn.init.constant_(layers["logvar_out"].weight, 1.0 / n_pop)
        self.basis = nn.ModuleDict(layers)
