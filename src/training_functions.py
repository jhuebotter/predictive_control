    ...
    for i in pbar:
        ...
        for j in range(batch_size):
            ...
            state_batch[:, j, :] = torch.stack(
                [step[0].squeeze() for step in episode_batch[j][r : r + steps]]
            )
            action_batch[:, j, :] = torch.stack(
                [step[2].squeeze() for step in episode_batch[j][r : r + steps]]
            )
            next_state_batch[:, j, :] = torch.stack(
                [step[4].squeeze() for step in episode_batch[j][r : r + steps]]
            )
    ...
    **kwargs,
    ...
    for i in pbar:
        ...
        while len(episode_batch) < batch_size:
            sample_batch = memory.sample(
                batch_size
            )  # [sample, step, (state, target, action, next_state)]
            for episode in sample_batch:
        ...
        target_batch = torch.zeros(
            (warmup_steps, batch_size, target_dim), device=device
        )
        action_batch = torch.zeros(
            (warmup_steps, batch_size, action_dim), device=device
        )

        for j in range(batch_size):
            r = torch.randint(
                low=0, high=len(episode_batch[j]) - warmup_steps, size=(1,)
            )
            state_batch[:, j, :] = torch.stack(
                [step[0].squeeze() for step in episode_batch[j][r : r + warmup_steps]]
            )
            target_batch[:, j, :] = torch.stack(
                [step[1].squeeze() for step in episode_batch[j][r : r + warmup_steps]]
            )
            action_batch[:, j, :] = torch.stack(
                [step[2].squeeze() for step in episode_batch[j][r : r + warmup_steps]]
            )
